...
/*
  Ptr<WeibullRandomVariable> uv = CreateObject<WeibullRandomVariable> ();
  uv->SetAttribute ("Scale", DoubleValue (1.0));
  uv->SetAttribute ("Shape", DoubleValue (0.8));
  uv->SetAttribute ("Bound", DoubleValue (0.1));
*/
  ApplicationContainer::Iterator appIt;
  for (appIt = senderApps.Begin (); appIt != senderApps.End (); ++appIt)
    {
      (*appIt)->SetStartTime (Seconds (0.2));
      //(*appIt)->SetStartTime (Seconds (uv->GetValue ()));
    }
