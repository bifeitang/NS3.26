...
main (int argc, char *argv[])
{
  uint16_t simTime = 10;
  bool verbose = false;
  std::string aredLinkDataRate = "1Mbps";
  std::string aredLinkDelay = "20ms";
  std::string EdgeClientDataRate = "10Mbps";
  std::string EdgeClientLinkDelay = "2ms";
  ...
  for(int pod = 0; pod < KARY; pod++)
  {
      for(int i= 0; i < KARY/2; i++)
      {
          ...
          {
        	  csmaHelper.SetQueue("ns3::DropTailQueue");
        	  csmaHelper.SetChannelAttribute ("DataRate", StringValue (EdgeClientDataRate));
        	  csmaHelper.SetChannelAttribute ("Delay", StringValue (EdgeClientLinkDelay));
  ...
  */
  InetSocketAddress rmt (ClientInterfaces.GetAddress(0), port);
  rmt.SetTos(0x28);
  OnOffHelper SenderHelper("ns3::TcpSocketFactory", Address(rmt));
